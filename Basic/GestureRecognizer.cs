...
public class GestureRecognizer
{
	...
	public static string skillSendOut; // sent out the skill name
    ...
    static void gestureMatch(ArrayList pointArray) 
    {
	    ...
	    if (score < 0.7f)
        {
		    ...
		    Gesture.GuiText.GetComponent<Text>().text = "RESULT: NO MATCH " +  "\n" + "SCORE: " + Mathf.Round(100 * score) +"%";
			skillSendOut = "NoMatch";

	    } else {
		    Debug.Log("RESULT: " + GestureTemplates.TemplateNames[count] + " SCORE: " + score);
		    Gesture.GuiText.GetComponent<Text>().text = "RESULT: " + GestureTemplates.TemplateNames[count] + "\n" + "SCORE: " + Mathf.Round(100 * score) +"%";
           

            skillSendOut = (string)GestureTemplates.TemplateNames [count];
	    }
